...
const WelcomePage = () => {
  ...
  return (
    <SafeAreaView className='flex h-full items-center justify-between bg-white'>
      ...
      >
        <Text className='text-md font-JosefinBold text-black'>Skip</Text>
      </TouchableOpacity>
      <Swiper
        ref={swiperRef}
        loop={false}
        ...
        {onboarding.map((item) => (
          <View key={item.id} className='flex items-center justify-center p-5'>
            ...
            <View className='mt-10 flex w-full flex-row items-center justify-center'>
              <Text className='font-JosefinBold mx-5 text-3xl text-black'>
                {item.title}
              </Text>
            </View>
            <Text className='text-md font-JosefinSemiBold mx-5 mt-3 text-center text-[#858585]'>
              {item.description}
            </Text>
          </View>
        ))}
